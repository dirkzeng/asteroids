...
public abstract class Sprite implements Movable {
	...
	private boolean bHasExploded = false;
	...
	public void explode(CopyOnWriteArrayList<Movable> movAddDebrisTo, boolean bMakeDebris) {
		if (bMakeDebris) {
			if (CommandCenter.movDebris.size() < CommandCenter.MAX_DEBRIS) {
				Debris deb = new Debris(this);
				movAddDebrisTo.add(deb);
	...
	public void explode(boolean bMakeDebris) {
		explode(CommandCenter.movDebris, bMakeDebris);
	}
	
	public void explode() {
		explode(CommandCenter.movDebris, true);
	}
    ...
    public void draw(Graphics g) {
        ...
        //record old background color
        Color oldColor = g.getColor();
        
        //switch to Graphics2D
        Graphics2D g2 = (Graphics2D) g;
        
        g2.setStroke(new BasicStroke(getLineWidth()));
        //draw background
        g2.setColor(getBackgroundColor());
        g2.fillPolygon(getXcoords(), getYcoords(), dDegrees.length);
        
        //get a thicker line width
        //draw outline
        g2.setColor(getColor());
        g2.drawPolygon(getXcoords(), getYcoords(), dDegrees.length);
